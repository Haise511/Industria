...
| GET | /users/:id | Публичный профиль (id/name/role/city/bio/avatar/rating/socials/streamings/cases) |
...
| **Профиль расширенный** | ✅ Готово | Бэк: `User.socials/streamings/cases` (JSON arrays of `{label,url}`), санитайз и валидация в `PUT /profile`; `GET /users/:id` — публичный subset профиля. Фронт: `/profile/edit` (`Личное.png`) с bio/городом/кейсами/именем; `/users/:id` (`Профиль/Профиль.png`) — публичный просмотр чужого профиля с описанием, ссылками и встроенным списком отзывов; `/reviews` (`Отзывы.png`) — мои полученные отзывы. Кликабельный автор в OrderDetail → переход на его профиль. Компонент `ReviewCard` переиспользуется в обоих экранах. |
