/*
 * vuesax/bold/send-sqaure-2 — the exact glyph from the Figma source
 * (node I1:6202;457:44083). The Send2 icon shipped by iconsax-react renders
 * a different vector, so we inline the path from the Figma export.
 *
 * Single-color glyph driven by `currentColor` so the BottomNav active-tab
 * color swap propagates without per-icon props.
 */
interface Props {
  size?: number;
}

export function SendSquare2({ size = 24 }: Props) {
  return (
    <svg
      width={size}
      height={size}
